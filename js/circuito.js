class Circuito {
  ...
  /**
   * Inicializa los event listeners para los inputs de archivo.
   */
  inicializarInput() {
  ...
  /**
   * Lee un archivo HTML seleccionado por el usuario.
   */
  ...
  /**
   * Parsea el contenido HTML y lo inserta en la página actual.
   */
...
class CargadorSVG {
  ...
  insertarSVG(contenidoSVG) {
    ...
    // Asegurar que el SVG tenga viewBox para responsividad
    if (!svg.hasAttribute("viewBox")) {
      svg.setAttribute("viewBox", "0 0 1200 400");
    }
    //Modificación del atributo version
...
class CargadorKML {
  ...
  leerArchivoKML(evento) {
    ...
    lector.onload = () => {
      ...
      // Buscar el elemento que contiene las coordenadas
  ...
  insertarCapaKML() {
    // Crear y dibujar la polilínea de la ruta
    ...
    // Añadir marcador en el punto de origen
