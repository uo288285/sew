class Carrusel {
  ...
  #getFotografias() {
    ...
    return $.getJSON(flickrAPI, {
      tags: this.#busqueda,
      tagmode: "all", //para que tenga los dos tags de la búsqueda
      format: "json",
    }).then((data) => {
  ...
  mostrarFotografias() {
    ...
    this.#getFotografias().then((fotografias) => {
      ...
      this.#$imagen.attr("src", primera.url).attr("alt", primera.titulo);
  ...
  #cambiarFotografia() {
    ...
    this.#$imagen.attr("src", foto.url).attr("alt", foto.titulo);
  }
}
