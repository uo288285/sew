...
class Test
{
    ...
    private function guardarRespuestas()
    {
        ...
        $incompleto = false;
        for ($i = 1; $i <= 10; $i++) {
            if (!isset($respuestas[$i]) || trim($respuestas[$i]) === '') {
                $respuestas[$i] = 'preguntaNoContestada';
                $incompleto = true;
            } else {
                $respuestas[$i] = trim($respuestas[$i]);
            }
        }
        $this->config->guardarRespuestasTest($respuestas);
        $tiempo = (int) $this->cronometro->getTiempo();
    ...
    ';
        ...
        <textarea id="comentarios" name="comentarios" rows="6" cols="60" placeholder="Escribe aquí tus comentarios sobre la aplicación si tienes..."></textarea>
    ';

            echo "<h3>Propuestas para mejorar la aplicación</h3>";
            echo '
        <label for="propuestas">Propuestas:</label>
        <textarea id="propuestas" name="propuestas" rows="6" cols="60" placeholder="Escribe aquí tus propuestas de mejora..."></textarea>
    ';

            echo "<h3>Comentarios del observador</h3>";
            echo '
        <label for="observacion">Observaciones:</label>
        <textarea id="observacion" name="observacion" rows="6" cols="60" placeholder="Escribe aquí las observaciones del facilitador..."></textarea>
    ';
